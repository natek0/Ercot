...
**Status: SUBSTANTIALLY COMPLETE** (see `reports/stage4_notes.md`). Energy side done in full and verified: the periodic DP (`src/dp.py`) passes the core gate (DP↔LP agreement <0.3% at 1–2h, Bellman residual ~0, μ monotone in S⁺, grid convergence). The **realized value-of-foresight ladder** (§IV.13): ceiling $13,206 ≥ clairvoyant MPC $12,847 ≥ **DP V^DP $5,346 (40% of ceiling)** ≥ learned MPC −$303 ≥ naive −$3,453 ≥ floor −$5,267, with **option value (V^DP − learned MPC) = +$5,650** — the DP's distribution-awareness turns the certainty-equivalent MPC's loss into profit (the result the DP exists to produce). **Q3** duration curves are concave with capture rising 13%→~49%. **§V.26 capture-rate prong resolved → adopt the empirical kernel** (empirical 40% > learned 33% realized capture; the learned CRPS win did not translate to better decisions — the publishable §V.26 negative result). **§VIII.7** state augmentation: the scarcity-regime coordinate z earns its place (+10.3% in-model). **Q2** causal ψ_up (2A) median $0.231 exceeds the Stage 0 perfect-foresight floor $0.015, validating Decision 19; the 2A hour-mean MCPC smooths the tail (a named refinement). Remaining refinements (not blocking): reserve-co-optimized DP ψ_up + the §IV.11 FD-vs-multiplier identity; interval MCPC for the ψ_up tail; a walk-forward kernel for V^DP; (h,b,z) in the realized policy; the summer top-up; c_deg sensitivity + the pre/post-RTC+B duration experiment.
