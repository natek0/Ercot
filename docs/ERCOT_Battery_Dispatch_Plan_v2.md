...
**Status: COMPLETE + adversarially reviewed** (see `reports/stage3_notes.md`). GBT quantile regression beat both baselines on held-out CRPS (learned 2.718 < jump 2.928 < empirical 5.253, 4/5 folds; beats empirical even at its best bin count 4.703) → **provisionally adopted on the CRPS prong**; the §V.26 capture-rate prong needs a policy and is evaluated in Stage 4. Gate MET with a *characterised* deviation, now separated into its two parts: the original deseasonaliser dropped the §V.26 `month` term, causing a seasonal level bias (fixed with a persistence-extrapolated month level; PIT-KS 0.031→0.024, tails symmetric); a genuine mild under-dispersion remains (+22% end-bin excess) → biases the Stage 4 policy to under-hold for spikes (toward MPC); tail inflation + a non-clamping-PIT re-check before freezing the kernel is a Stage 4 action. Hour-indexed transition matrices (tail-refined empirical-quantile bins) are row-stochastic; the matrix is irreducible via a small prior while the raw observed process is not on thin hours (reported honestly). Swapped into the certainty-equivalent MPC the learned forecast recovered $3,150 = 19% of the $16,300 forecast-error cost (median only; the tail value is Stage 4's). A three-agent adversarial review (math / trader / model-risk) plus synthesizer returned Yes-with-fixes, no blockers; all applicable fixes are applied.
...
**Status: COMPLETE and LEAK-FREE** (see `reports/stage4_notes.md` and `reports/stage4_review.md` once reviewed). The first cut had two integrity gaps, both now fixed after a critical-review pass: (Q1) the kernel is re-fit WALK-FORWARD per fold (was full-window in-sample, which inflated V^DP ~2.4× and violated §VIII.3); (Q2) ψ_up comes from the fully reserve-CO-OPTIMISED DP validated by the §IV.11-style C1 identity (was a post-hoc proxy). The periodic DP (`src/dp.py`) passes the core gate (DP↔LP <0.3% at 1–2h, Bellman ~0, μ monotone, grid convergence). **Corrected leak-free ladder** (§IV.13, 2h): ceiling $13,206 ≥ clairvoyant MPC $12,847 (97%) ≥ **DP V^DP $2,195 (17%)** ≥ learned MPC −$303 ≥ naive −$3,453 ≥ floor −$5,267; **option value (V^DP − learned MPC) = +$2,498** (positive — the DP still beats the CE MPC, but modestly; the in-sample cut's +$5,650 was inflated). Value of information $11,011. **Q3** curves concave, walk-forward capture rising from −5% (0.5h) to a ~22% plateau (≥2h). **§V.26 prong (walk-forward): empirical 17% > learned 15% → adopt the empirical kernel** — the conclusion survives the leak-free correction. **Q2** causal ψ_up from the co-optimised DP (C1 validation 5/5): at ρ=0 median $0.020 exceeds the Stage 0 floor $0.015 (Decision 19 validated), falling to 0 as ρ_k rises; the B2 hour-mean MCPC smooths the ψ_up tail (max <$1.2 vs Stage 0 $32.75 — a named refinement). The 17% capture is honest and information-limited (price-only kernel, no NWP, thin winter/spring window), not a bug (all gates pass). Remaining refinements (not blocking): interval MCPC for the ψ_up tail; fully-joint reserve concavity (the A2 precompute-and-add breaks μ-monotonicity ~1%); (h,b,z) walk-forward; NWP features + summer data; c_deg sensitivity + pre/post-RTC+B duration experiment.
